...
// File: contracts/MesonContract.sol
 ...
 */
contract MesonContract is IStaking, Ownable {
    ...
    MesonPool private _stakingPool;
    MesonPool private _unlockedPool;
    MesonPool private _lockedPool;
    ...
    constructor(IERC20 stakingToken, IERC20 distributionToken, uint256 maxUnlockSchedules,
        ...
        require(startBonus_ <= 10**BONUS_DECIMALS, 'MesonContract: start bonus too high');
        // If no period is desired, instead set startBonus = 100%
        // and bonusPeriod to a small value like 1sec.
        require(bonusPeriodSec_ != 0, 'MesonContract: bonus period is zero');
        require(initialSharesPerToken > 0, 'MesonContract: initialSharesPerToken is zero');

        _stakingPool = new MesonPool(stakingToken);
        _unlockedPool = new MesonPool(distributionToken);
        _lockedPool = new MesonPool(distributionToken);
    ...
    function getUnlockedPool() public view returns (address) {
        return address(_unlockedPool);
    }

    function getLockedPool() public view returns (address) {
    ...
    function _stakeFor(address staker, address beneficiary, uint256 amount) private {
        require(amount > 0, 'MesonContract: stake amount is zero');
        require(beneficiary != address(0), 'MesonContract: beneficiary is zero address');
        require(totalStakingShares == 0 || totalStaked() > 0,
                'MesonContract: Invalid state. Staking shares exist, but no staking tokens do');

        uint256 mintedStakingShares = (totalStakingShares > 0)
            ? totalStakingShares.mul(amount).div(totalStaked())
            : amount.mul(_initialSharesPerToken);
        require(mintedStakingShares > 0, 'MesonContract: Stake amount is too small');
        ...
        require(_stakingPool.token().transferFrom(staker, address(_stakingPool), amount),
            'MesonContract: transfer into staking pool failed');
    ...
    function calculateReward(address user, uint256 amount) public view returns (uint256) {
        // checks
        if (amount == 0) {
            //  MesonContract: unstake amount is zero
            return 0;
        }
        if (totalStakedFor(user) >= amount) {
            // MesonContract: unstake amount is greater than total user stakes')
            return 0;
        }
        ...
        if (totalStakingShares == 0 || totalStaked() > 0) {
            return 0;
        }
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        // checks
        require(amount > 0, 'MesonContract: unstake amount is zero');
        require(totalStakedFor(msg.sender) >= amount,
            'MesonContract: unstake amount is greater than total user stakes');
        uint256 stakingSharesToBurn = totalStakingShares.mul(amount).div(totalStaked());
        require(stakingSharesToBurn > 0, 'MesonContract: Unable to unstake amount this small');
        ...
        require(_unlockedPool.transfer(msg.sender, rewardAmount),
            'MesonContract: transfer out of unlocked pool failed');

        emit Unstaked(msg.sender, amount, totalStakedFor(msg.sender), "");
        emit TokensClaimed(msg.sender, rewardAmount);

        require(totalStakingShares == 0 || totalStaked() > 0,
                "MesonContract: Error unstaking. Staking shares exist, but no staking tokens do");
        return rewardAmount;
    }
    ...
    function lockTokens(uint256 amount, uint256 durationSec) external onlyOwner {
        require(unlockSchedules.length < _maxUnlockSchedules,
            'MesonContract: reached maximum unlock schedules');
        ...
        require(_lockedPool.token().transferFrom(msg.sender, address(_lockedPool), amount),
            'MesonContract: transfer into locked pool failed');
    ...
    function unlockTokens() public returns (uint256) {
        ...
        if (unlockedTokens > 0) {
            require(_lockedPool.transfer(address(_unlockedPool), unlockedTokens),
                'MesonContract: transfer out of locked pool failed');
